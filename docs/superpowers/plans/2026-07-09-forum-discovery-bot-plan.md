...
- Produces: `intents = discord.Intents(...)` module-level constant documented in README; `async def reconcile_on_startup(db: Database, gateway, guild_id: int) -> None` (runs `backfill_forum` for every monitored forum, resumes pending announcements via `NewThreadAnnouncer.process_due_announcements`); `def build_bot(config: Config, db: Database) -> commands.Bot` wiring all cogs and the APScheduler job. `on_ready` is not guaranteed by discord.py to fire only once (it can re-fire after a reconnect), so `build_bot` guards scheduler creation with a `bot.scheduler is None` check — a second `on_ready` firing must not start a second concurrent scheduler (which would double-post the digest and double-run the poll job). Both recurring jobs (`scheduled_digest_job`, `new_thread_poll_job`) wrap each guild's per-iteration body in its own `try/except`, mirroring `reconcile_on_startup`'s per-guild isolation in `on_ready` — one guild's failure must not skip every subsequent guild for the rest of that job's tick.
